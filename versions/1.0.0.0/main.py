...
def clear_text(page: ft.Page):
  ...
  for control in text_controls:
    if control != globals().get('input_field') and control != globals().get('button') and control != globals().get('clear_button'):
...
def main(page: ft.Page):
  ...
  global input_field, button, clear_button  # Declare all three globally
  ...
  def button_click(e):
    ...
    if url.lower().endswith((".png", ".jpg")):
      ...
      try:
        ...
        if text:
          # Text found, display extracted text with detected language
          page.add(ft.Text(f"Texto extraído ({lang}): {text}"))
        else:
          # No text found in any language
          page.add(ft.Text("Sem texto algum na imagem."))

        # Delete old PNGs except the latest (after OCR)
        delete_old_pngs(filename)

      except Exception as e:
        page.add(ft.Text(f"Erro ao baixar a imagem: {str(e)}"))  # Error message
    else:
      page.add(ft.Text("A URL precisa terminar com .png ou .jpg"))  # Invalid URL message
